...
const ACTION_HIGHLIGHT_COLORS: Record<BranchThread["action"], { bg: string; pill: string }> = {
  branch: { bg: "bg-emerald-500/20", pill: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" },
  ...
  connect: { bg: "bg-amber-500/20", pill: "bg-amber-500/10 text-amber-400 border-amber-500/20" },
};
...
function ThreadNavigator({
  ...
  const filterOptions: { key: BranchThread["action"] | "all"; label: string; color: string }[] = [
    ...
    { key: "branch", label: "Branch", color: "bg-emerald-500" },
    { key: "challenge", label: "Challenge", color: "bg-red-500" },
    { key: "define", label: "Define", color: "bg-blue-500" },
    { key: "connect", label: "Connect", color: "bg-amber-500" },
  ];
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-40 max-h-[85vh] bg-[#1f2c34] rounded-t-2xl border-t border-[#2a3942] flex flex-col"
      >
        {/* Handle + header */}
        <div className="flex-shrink-0 pt-3 pb-2 px-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="w-7 h-7 rounded-full bg-[#0b141a] flex items-center justify-center text-zinc-400 hover:text-zinc-200 transition-colors">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium transition-colors whitespace-nowrap ${
                    isActive ? "bg-[#2a3942] text-zinc-100" : "bg-[#0b141a] text-zinc-400 hover:bg-[#2a3942]/60"
                  }`}
                >
              ...
              {sorted.map((thread) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left p-3 rounded-xl bg-[#0b141a]/60 border border-[#2a3942] hover:bg-[#0b141a] transition-colors active:scale-[0.98]"
                  >
      ...
      <header className="sticky top-0 z-20 bg-[#1f2c34] border-b border-zinc-800">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {threads.length > 0 && (
              <button
                onClick={() => { setShowThreadNav(true); setToolbar(null); }}
                className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 active:scale-95 transition-all"
              >
                ⑂ {threads.length}
              </button>
            )}
        ...
        {showThreadNav && (
          <ThreadNavigator
            ...
            onSelectThread={(threadId) => {
              const buildFocusPath = (tid: string): string[] => {
                const thread = threads.find((t) => t.id === tid);
                if (!thread || !thread.parentThreadId) return [tid];
                return [...buildFocusPath(thread.parentThreadId), tid];
              };
              setFocusStack(buildFocusPath(threadId));
              setShowThreadNav(false);
            }}
            onClose={() => setShowThreadNav(false)}
            filter={threadNavFilter}
            onFilterChange={setThreadNavFilter}
          />
        )}
      </AnimatePresence>
